<script lang="ts">
    ...
    let delete_warning : HTMLDialogElement = $state();

</script>
<form method="POST" enctype="multipart/form-data" action={newRecipe ? "/recipes" : "?/update"} onsubmit={updateSteps}>
    <fieldset>
    ...
    <label class="form-control" for="steps">
        ...
        {#each data.steps as step, i}
            <div class="flex" data-id={i}>
                ...
                <button class="btn btn-ghost" type="button" onclick={function () {data.steps.splice(i, 1); data.steps = data.steps;}}>
    ...
    <div class="container my-4">
        ...
        <dialog bind:this={delete_warning} class="modal modal-bottom sm:modal-middle">
            <div class="p-4 modal-box">
                ...
                <button class="btn btn-secondary" onclick={preventDefault(() => delete_warning.close())}>No</button>
            </div>
        </dialog>
        {/if}
    </div>
